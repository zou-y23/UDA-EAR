...
class BasicVideoDataset(VideoFrameDataset):
    ...
    def __init__(
        ...
        self.n_classes_n = n_classes_n
        self.n_classes_v = n_classes_v
    ...
    def make_dataset(self):
        ...
        for line in input_file.values:
            if line[1] in ["P01", "P08", "P22"]:
                if 0 <= line[9] < self.n_classes_v and 0 <= line[11] < self.n_classes_n:
                    if line[7] - line[6] + 1 >= self.frames_per_segment:
                        label_v = line[9]
                        label_n = line[11]
                        data.append((os.path.join(line[1], line[2]), line[6], line[7], label_n, label_v))
                        i = i + 1
        logging.info("Number of {:5} action segments: {}".format(self.dataset, i))
        return data
