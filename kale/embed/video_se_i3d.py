...
from kale.embed.video_selayer import get_selayer, SELayerC, SELayerT, LayerCT, LayerST, LayerTAM
...
class SEInceptionI3DRGB(nn.Module):
    ...
    def __init__(self, num_channels, num_classes_n, num_classes_v, attention):
        super(SEInceptionI3DRGB, self).__init__()
        model = InceptionI3d(in_channels=num_channels, num_classes_n=num_classes_n, num_classes_v=num_classes_v)
        temporal_length = 16
        ...
        # Add channel-spatial-temporal-wise Layer
        elif attention == "LayerCST":
            model.Mixed_3b.add_module(attention + "ct", LayerCT(256))
            model.Mixed_3c.add_module(attention + "ct", LayerCT(480))
            model.Mixed_4b.add_module(attention + "ct", LayerCT(512))
            model.Mixed_4c.add_module(attention + "ct", LayerCT(512))
            model.Mixed_4d.add_module(attention + "ct", LayerCT(512))
            model.Mixed_4e.add_module(attention + "ct", LayerCT(528))
            model.Mixed_4f.add_module(attention + "ct", LayerCT(832))
            model.Mixed_5b.add_module(attention + "ct", LayerCT(832))
            model.Mixed_5c.add_module(attention + "ct", LayerCT(1024))

            # model.Mixed_3b.add_module(attention + "st", LayerST(256))
            # model.Mixed_3c.add_module(attention + "st", LayerST(480))
            # model.Mixed_4b.add_module(attention + "st", LayerST(512))
            # model.Mixed_4c.add_module(attention + "st", LayerST(512))
            # model.Mixed_4d.add_module(attention + "st", LayerST(512))
            # model.Mixed_4e.add_module(attention + "st", LayerST(528))
            # model.Mixed_4f.add_module(attention + "st", LayerST(832))
            # # model.Mixed_5b.add_module(attention + "st", LayerST(temporal_length // 8))
            # # model.Mixed_5c.add_module(attention + "st", LayerST(temporal_length // 8))

            model.Mixed_3b.add_module(attention + "st", LayerST(temporal_length // 2))
            model.Mixed_3c.add_module(attention + "st", LayerST(temporal_length // 2))
            model.Mixed_4b.add_module(attention + "st", LayerST(temporal_length // 4))
            model.Mixed_4c.add_module(attention + "st", LayerST(temporal_length // 4))
            model.Mixed_4d.add_module(attention + "st", LayerST(temporal_length // 4))
            model.Mixed_4e.add_module(attention + "st", LayerST(temporal_length // 4))
            model.Mixed_4f.add_module(attention + "st", LayerST(temporal_length // 4))
            # model.Mixed_5b.add_module(attention + "st", LayerST(temporal_length // 8))
            # model.Mixed_5c.add_module(attention + "st", LayerST(temporal_length // 8))
        ...
        # Add Triplet Attention Layer
        elif attention == "LayerTAM":
            # model.Mixed_3b.add_module(attention + "ta", LayerTAM(256, 28))
            model.Mixed_3c.add_module(attention + "ta", LayerTAM(480, 28))
            # model.Mixed_4b.add_module(attention + "ta", LayerTAM(512, 14))
            # model.Mixed_4c.add_module(attention + "ta", LayerTAM(512, 14))
            # model.Mixed_4d.add_module(attention + "ta", LayerTAM(512, 14))
            # model.Mixed_4e.add_module(attention + "ta", LayerTAM(528, 14))
            # model.Mixed_4f.add_module(attention + "ta", LayerTAM(832, 14))
            # model.Mixed_5b.add_module(attention + "ta", LayerTAM(832, 14))
            # model.Mixed_5c.add_module(attention + "ta", LayerTAM(1024, 14))
...
def se_inception_i3d(name, num_channels, num_classes_n, num_classes_v, attention, pretrained=False, progress=True, rgb=True):
    """Get InceptionI3d module w/o SELayer and pretrained model."""
    if rgb:
        model = SEInceptionI3DRGB(num_channels, num_classes_n, num_classes_v, attention)
    else:
        model = SEInceptionI3DFlow(num_channels, num_classes_n, num_classes_v, attention)
...
def se_i3d_joint(rgb_pt, flow_pt, num_classes_n, num_classes_v, attention, pretrained=False, progress=True):
    ...
    if rgb_pt is not None:
        i3d_rgb = se_inception_i3d(
            name=rgb_pt,
            num_channels=3,
            num_classes_n=num_classes_n,
            num_classes_v=num_classes_v,
            attention=attention,
