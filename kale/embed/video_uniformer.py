...
from kale.embed.build import MODEL_REGISTRY
...
class Uniformer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        num_classes_v = cfg.MODEL.NUM_CLASSES_V 
        num_classes_n = cfg.MODEL.NUM_CLASSES_N
        ...
        if split:
            ...
            self.blocks4 = nn.ModuleList([
                SABlock(
                    ...
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth[0]+depth[1]+depth[2]], norm_layer=norm_layer)
            for i in range(depth[3])])
            # self.blocks5 = nn.ModuleList([
            #     SABlock(
            #         dim=embed_dim[4], num_heads=num_heads[4], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
            #         drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth[0]+depth[1]+depth[2]], norm_layer=norm_layer)
        ...
        # Classifier head
        self.head1 = nn.Linear(embed_dim[-1], num_classes_v) if num_classes_v > 0 else nn.Identity()
        # head2
        self.head2 = nn.Linear(embed_dim[-1], num_classes_n) if num_classes_n > 0 else nn.Identity()
        ...
        for name, p in self.named_parameters():
            ...
            if 't_attn.proj.bias' in name:
                nn.init.constant_(p, 0)
        self.evidence = 'exp'
        self.out_layer = DirichletLayer(self.evidence, dim=-1) # new add
    ...
    def get_classifier1(self):
        return self.head1

    def get_classifier2(self):
        return self.head2

    def reset_classifier(self, num_classes_v, num_classes_n, global_pool=''):
        self.num_classes_v = num_classes_v
        self.num_classes_n = num_classes_n
        self.head1 = nn.Linear(self.embed_dim, num_classes_v) if num_classes_v > 0 else nn.Identity()
        self.head2 = nn.Linear(self.embed_dim, num_classes_n) if num_classes_n > 0 else nn.Identity()
    ...
    def get_pretrained_model(self, cfg):
        if cfg.UNIFORMER.PRETRAIN_NAME:
            ...
            if self.num_classes_v != checkpoint['head1.weight'].shape[0]:
                del checkpoint['head1.weight'] 
                del checkpoint['head1.bias'] 

            if self.num_classes_n != checkpoint['head2.weight'].shape[0]:
                del checkpoint['head2.weight'] 
                del checkpoint['head2.bias'] 
            
            return checkpoint # head1, head2 ? 
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks4):
            if self.use_checkpoint and i < self.checkpoint_num[3]:
                ...
                x = blk(x)
        # for i, blk in enumerate(self.blocks5):
        #     if self.use_checkpoint and i < self.checkpoint_num[3]:
        #         x = checkpoint.checkpoint(blk, x)
        #     else:
        #         x = blk(x)
    ...
    def forward(self, x):
        # x = x[0]
        x = self.forward_features(x) # 提特征
        # x = x.flatten(2).mean(-1)
        # x1 = self.head1(x) # 分类任务, 输出x1
        x1 = x
        # x2 = self.head2(x) # 再加一个head做分类任务，输出x2
        x2 = x
        # unct1 = self.out_layer.compute_uncertainty(x1) # unct1对应x1的
        # unct2 = self.out_layer.compute_uncertainty(x2) # 加一个unct2对应x2的
        return x1, x2
