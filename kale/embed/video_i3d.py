...
class InceptionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        if "LayerCSTct" in dir(self):    # check channel-spatial-temporal-wise
            out = self.LayerCSTct(out)
            # print('LayerCSTct')
        if "LayerCSTst" in dir(self):
            out = self.LayerCSTst(out)
            # print('LayerCSTst')

        if "LayerTAMta" in dir(self):  # check triplet-wise
            out = self.LayerTAMta(out)
            # print('LayerTAMta')
        return out
...
class InceptionI3d(nn.Module):
    ...
    def __init__(
        ...
        self._num_classes_n = num_classes_n
        self._num_classes_v = num_classes_v
        ...
        self.build()
        # self.tmp_conv3d_layer = nn.Conv3d(480, 1024, kernel_size=1, stride=1, padding=0)
        self.tmp_conv3d_layer = nn.Conv3d(1024, 1024, kernel_size=1, stride=1, padding=0)
        self.tmp_pool_layer = nn.AdaptiveAvgPool3d((1, 1, 1))
    ...
    def forward(self, x):
        ...
        # tmp_x = x
        # tmp_x = self.tmp_conv3d_layer(tmp_x)
        # tmp_x = self.tmp_pool_layer(tmp_x)
