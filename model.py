...
def get_model(cfg, dataset, num_classes_n, num_classes_v):
    """
    ...
    feature_network, class_feature_dim, domain_feature_dim = get_video_feat_extractor(
        cfg.MODEL.METHOD.upper(), cfg.DATASET.IMAGE_MODALITY, cfg.MODEL.ATTENTION, num_classes_n, num_classes_v, cfg
    )
    device = torch.device('cuda:0')
    checkpoint = feature_network.get_pretrained_model(cfg)
    feature_network.load_state_dict(checkpoint, strict=False)
    feature_network.to(device)
    # setup classifier
    # classifier_network_n = ClassNetVideo(input_size=class_feature_dim, n_class=num_classes_n)
    # classifier_network_v = ClassNetVideo(input_size=class_feature_dim, n_class=num_classes_v)
    classifier_network_n = ClassNetVideo(input_size=512, n_class=num_classes_n)
    classifier_network_v = ClassNetVideo(input_size=512, n_class=num_classes_v)
    ...
    if method.is_mmd_method():
        ...
        print(critic_input_size)
        # critic_network = DomainNetVideo(input_size=critic_input_size)
        critic_network = DomainNetVideo(input_size=512)
        ...
        model = video_domain_adapter.create_dann_like_video(
            ...
            image_modality=cfg.DATASET.IMAGE_MODALITY,
            feature_extractor=feature_network,
            task_classifier_n=classifier_network_n,
            task_classifier_v=classifier_network_v,
