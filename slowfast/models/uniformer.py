...
class Uniformer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        num_classes_2 = cfg.MODEL.NUM_CLASSES_2 
        ...
        # Classifier head
        self.head1 = nn.Linear(embed_dim[-1], num_classes) if num_classes > 0 else nn.Identity()
        # head2
        self.head2 = nn.Linear(embed_dim[-1], num_classes_2) if num_classes_2 > 0 else nn.Identity()
    ...
    def reset_classifier(self, num_classes, num_classes_2, global_pool=''):
        self.num_classes = num_classes
        self.num_classes_2 = num_classes_2
        self.head1 = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.head2 = nn.Linear(self.embed_dim, num_classes_2) if num_classes_2 > 0 else nn.Identity()
    ...
    def get_pretrained_model(self, cfg):
        if cfg.UNIFORMER.PRETRAIN_NAME:
            ...
            if self.num_classes != checkpoint['head1.weight'].shape[0]:
                del checkpoint['head1.weight'] 
                del checkpoint['head1.bias'] 

            if self.num_classes_2 != checkpoint['head2.weight'].shape[0]:
